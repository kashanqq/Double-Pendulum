...
// --- NEW: Energy Calculation (Error Analysis) ---
float calculateEnergy(const State& s, const PhysicsParams& p) {
    // 1. Potential Energy (V = mgh)
    // Height is measured from the pivot point. y goes down in SFML, but up in physics.
    // y1 = -L1 * cos(a1)
    ...
    // 2. Kinetic Energy (T)
    // Formula for double pendulum kinetic energy
...
int main() {
    ...
    sf::String title("Kebab Engine: Error Analysis Lab");
    ...
    // --- Energy Tracking ---
    float initialEnergy = calculateEnergy(state, params);

    std::vector<sf::Vector2f> trailPoints;
    const float SCALE = 150.0f;
    const sf::Vector2f OFFSET(windowSize.x / 2.0f, windowSize.y / 3.0f); // Чуть выше центра
    ...
    while (window.isOpen()) {
        while (const std::optional event = window.pollEvent()) {
            ...
            if (const auto* keyPress = event->getIf<sf::Event::KeyPressed>()) {
                if (keyPress->code == sf::Keyboard::Key::R) {
                    state = initialState;
                    trailPoints.clear();
                    // Сбрасываем эталонную энергию при рестарте
                    initialEnergy = calculateEnergy(state, params);
                }
        ...
        if (changeSpeed != 0.0f) {
            ...
            // Если меняем параметры, энергия системы меняется извне. 
            // Чтобы "Error" не сходил с ума, обновляем эталон, пока пользователь жмет кнопку.
            initialEnergy = calculateEnergy(state, params);
        }
        ...
        float currentEnergy = calculateEnergy(state, params);
        ...
        sf::Text analysisTitle(font);
        analysisTitle.setString("--- ANALYSIS (RK4) ---");
        analysisTitle.setCharacterSize(18);
        ...
        ssAnalysis << "Status: " << (error < 0.1f ? "STABLE" : "DRIFT DETECTED");

        sf::Text analysisText(font);
        analysisText.setString(ssAnalysis.str());
        analysisText.setCharacterSize(16);
        analysisText.setFillColor(sf::Color::White);
        analysisText.setPosition({ 10.f, analysisY + 30.0f });

        // Подсветка статуса
        if (error > 0.1f) analysisText.setFillColor(sf::Color(255, 100, 100)); // Красный если ошибка большая
